...
Division APIは、6つの主要AIプロバイダーの145以上のモデルをサポートしています。  
...
| 🟣 **Claude (Anthropic)**   | 🟣 高精度で安全なAIモデル（Sonnet、Opus、Haiku）           |
| 🔵 **Gemini (Google)**      | 🔵 マルチモーダル対応の高性能モデル                         |
| 🟠 **Perplexity**           | 🟠 リアルタイム検索機能を備えたAI（Sonar Pro等）           |
| ⚫ **xAI**                  | ⚫ 高速で効率的なモデル（Grok等）                          |
| 🔴 **DeepSeek**             | 🔴 深い理解を得るための推論モデル（V3、R1等）              |
...
タスクの種類に応じて、最適なAIモデルが自動で切り替わります。各ロールは特定のタスク品質の最適化されています。
...
#### 🧑‍💻 Coder ロール
- **用途**: コード生成、実装、バグ修正、テスト作成
- **最適モデル**: Claude 3.5 Sonnet (精度重視)
- **フロー**: コンテキスト解析 → モデル選択 → コード生成 → 統合

#### 🔍 Searcher ロール
- **用途**: 情報検索、リサーチ、データ分析、市場調査
- **最適モデル**: Perplexity Sonar Pro (リアルタイム検索)
- **フロー**: クエリ理解 → リアルタイム検索 → 信頼性確認 → 結果提示

#### 🔎 Reviewer ロール
- **用途**: コードレビュー、セキュリティチェック、品質保証
- **最適モデル**: GPT-4 (複雑な分析)
- **フロー**: コード読込 → 詳細分析 → 改善提案 → レポート生成

#### 📋 Planner ロール
- **用途**: タスク計画、アーキテクチャ設計、スプリント計画
- **最適モデル**: Claude Opus (複雑な戦略立案)
- **フロー**: 要件解析 → タスク計画 → リソース配分 → ドキュメント生成
